...
export const NotificationPoller: React.FunctionComponent<NotificationPollerProps> = (props: NotificationPollerProps) => {
  ...
  const [n, setNotification] = useState(props.notification);
  ...
  useInterval(async () => {
    ...
    if (JSON.stringify(op) !== JSON.stringify(n.operation)) {
      let localNotification = n;
      localNotification.operation = op;
