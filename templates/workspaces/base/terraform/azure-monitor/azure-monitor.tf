resource "azurerm_log_analytics_workspace" "workspace" {
  name                       = "log-${var.tre_id}-ws-${var.unique_identifier_suffix}"
...
resource "azurerm_storage_account" "app_insights" {
  name                            = lower(replace("stai${var.tre_id}ws${var.unique_identifier_suffix}", "-", ""))
  resource_group_name             = var.resource_group_name
  location                        = var.location
...
resource "azurerm_monitor_private_link_scope" "workspace" {
  name                = "ampls-${var.tre_id}-ws-${var.unique_identifier_suffix}"
  resource_group_name = var.resource_group_name
  tags                = var.tre_workspace_tags
...
resource "azurerm_private_endpoint" "azure_monitor_private_endpoint" {
  name                = "pe-ampls-${var.tre_id}-ws-${var.unique_identifier_suffix}"
  resource_group_name = var.resource_group_name
  location            = var.location
  ...
  private_service_connection {
    ...
    name                           = "psc-ampls-${var.tre_id}-ws-${var.unique_identifier_suffix}"
