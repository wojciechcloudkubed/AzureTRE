...
resource "azurerm_linux_web_app" "guacamole" {
  ...
  app_settings = {
    ...
    OAUTH2_PROXY_REDIRECT_URI    = "https://${local.webapp_name}.azurewebsites.us/oauth2/callback"
    OAUTH2_PROXY_EMAIL_DOMAIN    = "\"*\"" # oauth proxy will allow all email domains only when the value is "*"
    OAUTH2_PROXY_OIDC_ISSUER_URL = "https://login.microsoftonline.us/${local.aad_tenant_id}/v2.0"
    OAUTH2_PROXY_JWKS_ENDPOINT   = "https://login.microsoftonline.us/${local.aad_tenant_id}/discovery/v2.0/keys"
  }
