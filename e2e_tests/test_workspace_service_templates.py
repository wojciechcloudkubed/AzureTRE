...
async def test_get_workspace_service_templates(template_name, verify) -> None:
    async with AsyncClient(verify=verify) as client:
        ...
        response = await client.get(f"https://{config.TRE_ID}.{config.RESOURCE_LOCATION}.cloudapp.usgovcloudapi.net{strings.API_WORKSPACE_SERVICE_TEMPLATES}", headers=get_auth_header(admin_token))
...
async def test_create_workspace_service_templates(verify) -> None:
    async with AsyncClient(verify=verify) as client:
        ...
        admin_token = await get_admin_token(verify)
        response = await client.post(f"https://{config.TRE_ID}.{config.RESOURCE_LOCATION}.cloudapp.usgovcloudapi.net{strings.API_WORKSPACE_SERVICE_TEMPLATES}", headers=get_auth_header(admin_token), json=payload)
