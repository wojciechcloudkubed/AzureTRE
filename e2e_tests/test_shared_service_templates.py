...
async def test_get_shared_service_templates(template_name, verify) -> None:
    async with AsyncClient(verify=verify) as client:
        ...
        response = await client.get(f"https://{config.TRE_ID}.{config.RESOURCE_LOCATION}.cloudapp.usgovcloudapi.net{strings.API_SHARED_SERVICE_TEMPLATES}", headers=get_auth_header(admin_token))
