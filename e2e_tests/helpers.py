...
def get_full_endpoint(endpoint: str) -> str:
    if (config.TRE_URL != ""):
        ...
        return f"https://{config.TRE_ID}.{config.RESOURCE_LOCATION}.cloudapp.usgovcloudapi.net{endpoint}"
...
async def get_admin_token(verify) -> str:
    async with AsyncClient(verify=verify) as client:
        ...
        if config.TEST_ACCOUNT_CLIENT_ID != "" and config.TEST_ACCOUNT_CLIENT_SECRET != "":
            ...
            url = f"https://login.microsoftonline.us/{config.AAD_TENANT_ID}/oauth2/v2.0/token"
            ...
            url = f"https://login.microsoftonline.us/{config.AAD_TENANT_ID}/oauth2/token"
