...
def get_azure_resource_status(resource_id):
    ...
    try:
        if resource_type == "Microsoft.Compute/virtualMachines":
            vm_instance_view: models.VirtualMachineInstanceView = (
                get_azure_vm_instance_view(resource_name, resource_group_name)
            )
            power_state = None
            if vm_instance_view.statuses is not None:
                power_states = [
                    x
                    for x in vm_instance_view.statuses
                    if x.code is not None and x.code.startswith("PowerState")
                ]
        ...
        logging.warning(
            f"Unable to query resource status for {resource_id}, as the resource was not found."
        )
